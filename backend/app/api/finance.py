...
def update_invoice(
    ...
    if not invoice:
        raise HTTPException(status_code=404, detail="Invoice not found")
    if current_user.role != UserRole.SUPER_ADMIN and invoice.branch_id != current_user.branch_id:
        raise HTTPException(status_code=403, detail="Not allowed to update this invoice")

    payload = body.dict(exclude_unset=True)
    manual_status = "status" in payload
    ...
    add_history(
        ...
        module="finance",
        action="updated",
        title=f"Invoice updated: {invoice.invoice_number}",
        details=f"Student ID: {invoice.student_id} | Amount: Rs {invoice.amount:.0f} | Status: {invoice.status}",
        record_id=invoice.id,
        created_by_id=current_user.id,
        branch_id=invoice.branch_id,
    )
...
def update_payment(
    ...
    if not invoice:
        raise HTTPException(status_code=404, detail="Invoice not found")
    if current_user.role != UserRole.SUPER_ADMIN and invoice.branch_id != current_user.branch_id:
        raise HTTPException(status_code=403, detail="Not allowed to update this payment")
    ...
    if "amount" in payload:
        invoice.paid_amount = max(0, (invoice.paid_amount or 0) - old_amount + payment.amount)
        update_invoice_status(invoice)
    ...
    add_history(
        ...
        title=f"Payment updated: Rs {payment.amount:.0f}",
        details=f"Invoice ID: {invoice.id} | Method: {payment.payment_method}",
        record_id=payment.id,
        created_by_id=current_user.id,
        branch_id=invoice.branch_id,
    )
...
def delete_payment(
    ...
    if not invoice:
        raise HTTPException(status_code=404, detail="Invoice not found")
    if current_user.role != UserRole.SUPER_ADMIN and invoice.branch_id != current_user.branch_id:
        raise HTTPException(status_code=403, detail="Not allowed to delete this payment")
    ...
    add_history(
        ...
        title=f"Payment deleted: Rs {payment.amount:.0f}",
        details=f"Invoice ID: {invoice.id} | Student ID: {payment.student_id}",
        record_id=payment.id,
        created_by_id=current_user.id,
        branch_id=invoice.branch_id,
    )
