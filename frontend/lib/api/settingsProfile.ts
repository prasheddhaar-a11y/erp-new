...
import { API_URL, apiRequest, getStoredSessionValue, parseRequestError, storeSessionValue } from "@/lib/api"
...
function cacheApiProfile(profile: ProfileApiResponse) {
  ...
  const currentUser = safeJsonParse<Record<string, unknown>>(currentUserRaw, {})
  const nextUser = { ...currentUser, ...profile }
  const storage = remember ? window.localStorage : window.sessionStorage

  storeSessionValue("pinesphere_profile", JSON.stringify(profile), remember)
  storage.setItem("pinesphere_user", JSON.stringify(nextUser))
}
