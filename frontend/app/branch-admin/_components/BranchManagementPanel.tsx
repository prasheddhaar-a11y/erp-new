...
export function BranchManagementPanel({ accessToken }: { accessToken: string }) {
  ...
  const loadBranches = useCallback(async (isAutoRefresh = false) => {
    ...
    setError("");
    try {
      const [branchRows, compareRows, capacityRows] = await Promise.all([
        apiRequest<BranchResponse[]>("/branches", accessToken),
      ...
      setStatsData(compareRows);
      setCapacityData(capacityRows);
      /* =====================================================
      ...
      setLiveSignals(await apiRequest<BranchLiveSignalsResponse>("/branches/live-signals", accessToken).catch(() => null));
      setNotice("");
    } catch (branchError) {
